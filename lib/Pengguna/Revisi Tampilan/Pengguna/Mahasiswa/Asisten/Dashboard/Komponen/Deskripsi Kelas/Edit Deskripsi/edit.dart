...
class _EditDeskripsiState extends State<EditDeskripsi> {
  ...
  void _saveDeskripsi() async {
    ...
    QuerySnapshot deskripsiSnapshot = await silabusCollection
        .where('idKelas', isEqualTo: widget.idkelas)
        .get();

    // Dokumen ditemukan, lakukan update
    DocumentReference docRef = deskripsiSnapshot.docs.first.reference;
    await docRef.update({
  ...
  Future<void> _loadUserData() async {
    try {
      ...
      if (userData.docs.isNotEmpty) {
        ...
        if (mounted) {
          setState(() {
            _hardwareController.text = data['perangkatKeras'] ?? '';
            _softwareController.text = data['perangkatLunak'] ?? '';
            _deskripsiKelasController.text = data['deskripsikelas'] ?? '';
          });
        }
      } else {
        if (mounted) {
          setState(() {
            _hardwareController.text = '';
            _softwareController.text = '';
            _deskripsiKelasController.text = '';
          });
        }
      }
